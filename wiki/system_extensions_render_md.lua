...
inlineParser = wikiParser(
	...
	"<%?lua ", function (remainder, m)
		return wikiParserMatched(remainder, m, "?>", true, function (code)
			...
			if not v1 then
				table.insert(contents, h("code", {}, v1))
			else
...
if props.inline then
	...
	local preservedBlocks = 0
	local function prepareForInsert()
		while #blockStack > preservedBlocks do
			closeTopBlock()
		end
	...
	local function openBlock(block)
		...
		preservedBlocks = #blockStack
	end
	local function forceCloseThisBlock(block)
	...
	for _, line in ipairs(lines) do
		preservedBlocks = 0
		for i = 1, #blockStack do
			local lineMod, preserve = blockStack[i]:handle(line)
			line = lineMod
			if preserve then
				preservedBlocks = preservedBlocks + 1
			else
				break
			end
		end
